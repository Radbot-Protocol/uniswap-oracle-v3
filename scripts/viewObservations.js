...
async function viewObservations() {
  ...
  try {
    ...
    let observationsLength;
    try {
      observationsLength = await oracle.pairObservations(PAIR_ADDRESS, 0);
    } catch (error) {
      console.log(
        "❌ Error accessing pairObservations. The pair might not exist or oracle might not be deployed."
      );
      return;
    }
...
async function getObservationsForTokenPair(tokenA, tokenB) {
  const RPC_URL = "https://mainnet.base.org"; // Base mainnet RPC
  const provider = new ethers.JsonRpcProvider(RPC_URL);
...
module.exports = { viewObservations, getObservationsForTokenPair };
